...
- Maintenance Drive folder (in-boundary, keyless DWD as a pmikcmetro.com user): `npm run maintenance:ensure-folder -- --live` — find-or-creates the photo folder + prints the id for SPACE_DRIVE_FOLDER_IDS. PREREQ: a Workspace admin must authorize the Drive scope for the DWD service account (Admin console → Domain-wide delegation), like the Sheets scope; until then --live returns unauthorized_client.
