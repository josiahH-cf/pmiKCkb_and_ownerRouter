...
// approval). The code is what the learning loop reads so decisions become machine-readable AND
// value-scrubbable (a code carries no client value). S14 also lets a code's label satisfy the audit
// reason for the narrow Low/Medium accept-suggested + one-tap approve path.
//
...
// records remain readable and manual decisions may continue using their required free-text reason.
...
export const DECISION_REASON_CODE_LABELS: Record<DecisionReasonCode, string> = {
  accepted_suggestion: "Accepted the suggested source",
