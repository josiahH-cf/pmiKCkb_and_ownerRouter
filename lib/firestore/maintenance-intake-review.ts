...
// the intake un-flipped (which would let it be promoted twice). The promoted ticket is marked
// reporter.kind="external", unit=null, labelled "Needs Verification" — an external report has no verified
// unit until a human confirms it.
...
export async function promoteUnverifiedIntake(
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const ticket: MaintenanceTicketRecord = {
      ...
      labels: [NEEDS_VERIFICATION_LABEL],
    ...
    const ticketActivity: MaintenanceTicketActivityRecord = {
      ...
      action: "create",
      new_status: "Open",
      text: "Promoted from a public intake report (unit needs verification).",
      created_at: timestamp,
    };
