...
```mermaid
flowchart TD
    A[Choose a lease and review its facts] --> B[Gather comps and prepare owner request]
    B --> C[Record actual owner response]
    C -->|Exact terms approved| D[Prepare tenant offer and record response]
    C -->|Waiting| W[Record what is missing and resume later]
    C -->|Declined| N[Record the approved non-renewal handoff]
    D -->|Counteroffer| C
    D -->|Declined| N
    D -->|Accepted| E[Prepare applicable documents and obtain signatures outside the app]
    E --> F[Record returned artifacts and applicable closeout work]
    F --> G[Record staff completion]
    G --> H[Return to the same filtered renewal list]
```
