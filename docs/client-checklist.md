...
| Gmail helper workflow model  | Confirm whether the plain-English organizing model is right: Waiting on Outside / Waiting on Team, then possibly Dan Decision / Draft Ready.                               | Later automated triage design.                      | Awaiting confirmation; not a blocker for user-invoked label application.                                                          | Record rules/exclusions before any automatic processing or historical scan.       |
...
| Gmail production activation  | COMPLETE 2026-07-13: DWD readonly/compose/labels/modify, all five Inbox 0 actions, Pub/Sub/watch, deployment, self-addressed proof, and repository publication.                  | Per-user live Gmail.                | Resources and identifiers are recorded in the S19 evidence; exact confirmation remains.                |
...
| Gmail Inbox 0 test access | Use the signed-in user's bounded inbox and a self-addressed synthetic proof. Dan mailbox scanning/automatic processing remains a later workflow step.                                       |
...
| Gmail Inbox 0       | RESOLVED for S19 production: reviewed recipients are supported; app send/reply requires exact payload review and explicit confirmation; no autonomous send.                                                                                              |
...
- Gmail Inbox 0 production activation is complete for the four exact scopes, five action keys,
  Pub/Sub/watch, deployment, and synthetic self-addressed proof. Dan mailbox scanning/automatic
  processing was not part of that activation.
