...
| Gmail helper workflow model  | Later, confirm whether the plain-English organizing model is right: Waiting on Outside / Waiting on Team, then possibly Dan Decision / Draft Ready.                        | Gmail Inbox 0 workflow rollout after the technical self-only proof. | Awaiting confirmation; not a blocker for the bounded owner-approved self-thread proof.                                            | Record labels/exclusions before any Dan scan, label mutation, or broader-recipient promotion. |
...
| Gmail readonly DWD grant     | COMPLETE 2026-07-13: the existing DWD client accepted exactly `https://www.googleapis.com/auth/gmail.readonly`; one keyless self-pilot profile call matched `josiah@pmikcmetro.com`, returned aggregate counts, and supplied a history cursor. Evidence records no token/content. | Bounded authenticated-user mailbox connection.             | Read action locally promoted; deployment and any thread/body read remain separate approvals.           |
| Gmail self-thread proof      | Choose one `pmikcmetro.com` pilot user, confirm the exact synthetic self-recipient subject/body at action time, and approve one send plus one reply. Dan/third-party delivery is excluded.                                                                                        | Safe proof of exact-confirmation send and Gmail threading. | Owner action required after local gates; record IDs/counts/status only.                                |
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Google Workspace admin    | For S19 Stage 1, add only `gmail.readonly` to DWD client `104374162913177846911`; do not add `gmail.modify` or `mail.google.com`. Later Gmail labels/filters remain a separate approval.    |
...
| Gmail Inbox 0 test access | First use one configured authenticated pilot's bounded inbox and self-addressed synthetic thread. Dan mailbox scan/testing is a later, separately approved workflow step.                   |
...
| Gmail Inbox 0       | RESOLVED for S19 technical pilot: unsent draft plus app send/reply only after the authenticated user reviews the exact payload and explicitly confirms; self-recipient only, no autonomous send. Broader recipients remain open.                         |
...
- Gmail Inbox 0 Stage 1 has a verified `gmail.readonly` self-pilot profile connection on DWD
  client `104374162913177846911`; `gmail.mailbox.read` is locally promoted. Send/reply keys remain
  false until an action-time synthetic self-thread proof is approved. Any thread/body read, Pub/Sub
  setup, and deploy are later, separately approved actions. Dan mailbox access was not part of the proof.
