...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| Google Cloud billing card          | Complete Google Cloud free-trial/billing setup for the `pmikcmetro.com` organization, create/confirm the production project, and keep the budget context at $10/no spend without approval.         | KB migration to client-owned cloud platform.        | Guidance sent to Dan; awaiting completion/project details.                                                                                                                                                                                                  | Billing visible/enabled, then run approved setup/preflight dry-runs.              |
...
| Lease Renewal walkthrough          | Record one full start-to-finish Lease Renewal walkthrough using the checklist sent by Josiah: trigger, exact source fields/screens, decision owners, updates, sends, blockers, and done state.     | Lease Renewal Agent discovery and prototype scope.  | HELD 2026-06-19 (live screen-share; Dan → Josiah/Jon); source questions answered (`docs/products/lease-renewal-discovery-reference.md`). No re-recording needed — optional only: a short screen-share of the exact RentVine renewal + rent-increase clicks. | Record/summarize walkthrough into source notes without raw client data in git.    |
...
| Signed lease and lease-date source | Identify the exact system, sheet, tab, column, screen, or document where signed leases and lease end dates live.                                                                                   | Lease Renewal Agent first authoritative trigger.    | RESOLVED [CERTAIN] (JH 2026-06-20): executed signed leases live in Dotloop; renewal timing / lease-end reads from the RentVine lease record (`lib/integrations/rentvine/lease-mapper.ts` → `lease_end_date`). See discovery-reference "Certainties".        | Source system classified as read-only/write-capable/blocked.                      |
