...
| Vendor auth/OAuth setup     | Approve Identity Platform TOTP, invitation-delivery lane, Google OAuth consent/client/redirect, token-vault resource, and one safe Vendor acceptance account/ticket.                                                                    | S22 external Vendor portal.                         | R04 behavior is settled; no live Vendor principal/OAuth app is configured by this audit.                                          | Prove invite→MFA→assigned ticket→OAuth→revoke before first send.                                                                                             |
| Workflow recipient mappings | Identify the authoritative runtime adapters/fields for renewal owner/tenant and maintenance owner/vendor recipients and required values.                                                                                                | S24/S25/S26 initiation.                             | R07 artifacts/AI policy are approved; browser-entered addresses remain non-authoritative.                                         | Render v1.0 instances and prove missing/drifted values block.                                                                                                |
...
| External Vendor Auth       | R04 contract settled; S22 still needs approved Identity Platform TOTP, invite delivery, OAuth app/redirect, token vault, and first acceptance account.                                      |
...
| Workflow Communications | RESOLVED policy: 10m/30d confirmation, 7d dedupe, 90d sync, 365d link, 7y bodyless audit, no persisted V1 AI facts, legal hold override. Implement S24.                                                                                                  |
...
- **No product-decision blocker remains from the audit.** Build S20–S27 in the ordered program with
  fake providers/emulators; do not wait for the client before safe local work.
...
- **Workflow Communications promotion:** implement S24 retention/legal hold and v1.0 artifact/AI policy,
  then configure authoritative recipient/value adapters. AI exclusions and watch ownership are resolved.
...
Final-V1 blockers until implemented/accepted: QuickBooks draft bill, LeadSimple, Dotloop, Boom, Sheet
writeback, Rentvine renewal/portal and Maintenance create/assign/update/close, Drive photo, SMS, workflow
Gmail initiation, Vendor auth/OAuth/mail, and all other S25/S26 rows. Generic Gmail inbox/compose,
