...
| Production       | Serving Live-only application                       | Project `pmi-kc-kb-prod` (#558870356522); Cloud Run `pmi-kc-app`, `us-central1`; canonical URL `https://pmi-kc-app-kq6wuvpiva-uc.a.run.app`; bucket `pmi-kc-kb-prod-sources-558870356522`; search store `kb-lease-renewals-txt`; Firebase app `1:558870356522:web:c1b2473b886a6edd889953` | Secret Manager / attached identities | Josiah technical; PMI KC business | Revision `pmi-kc-app-rmsol14wb-9fe02e7af754` serves 100%; captured predecessor `pmi-kc-app-rmsisg7di-1f914cfeae0d`. Production+Live readback, exact-tag/stable smoke, rollback, and forward restore passed; legacy service `pmi-kc-kb-demo` remains deleted/absent. |
...
| RentCast      | Market comps and rent estimates              | Active API plan, request budget, `MARKET_COMP_PROVIDER` selector                    | API key in Secret Manager; PMI KC             | RESOLVED 2026-08-06 (`Q-RENTCAST-PLAN-TERMS`): the [API Terms](https://www.rentcast.io/terms-api) expressly permit storing, caching, displaying, and distributing the data, with no attribution duty and no retention limit. Still owner-confirmed at the account: an ACTIVE API subscription (the placed key answers 403 until the plan is activated, `Q-RENTCAST-ACCOUNT-403`) and the real allowance/overage figures that size the app-side hard quota stop (AC-S59-14; documented free-plan default 50/month, errors unbilled, overage bills automatically). | Gated off; refusals are legible per reason; manual comp entry remains usable                                            |
...
On WSL, keep `GOOGLE_APPLICATION_CREDENTIALS` unset. Normal ADC discovery uses the verified symlink
documented in `docs/auth-identity-and-access-strategy.md`; gcloud CLI commands use
`CLOUDSDK_CONFIG=/mnt/c/Users/josia/AppData/Roaming/gcloud`, and Firebase CLI checks use
`XDG_CONFIG_HOME=/mnt/c/Users/josia/.config`. These select the refreshed managed Windows stores and
do not copy a credential or introduce a key-file override.
