...
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `google_sheets.renewal_checklist.writeback`  | Appends one approved value to one empty `KB Proposed — <field>` cell. The current `confirm:boolean` is not sufficient activation evidence: the key stays closed until commit requires the exact server-issued preview hash bound to actor, run, trigger, sheet, tab, cell, proposed value, approval version, and environment; drift or replay refuses before a writer call, and correction is receipted. | Create the `KB Proposed — Comp basis` column on the renewal sheet and confirm the exact spreadsheet id and tab name (D32).                                                 | Already forwards `LEASE_RENEWAL_SHEET_WRITEBACK_ENABLED`, `RENEWAL_SHEET_ID`, `SHEETS_IMPERSONATE_SA`, `SHEETS_DWD_SUBJECT`. ADD `ENVIRONMENT_KIND` and `DATA_CONTEXT` so the new descriptor fence is explicit, not inferred from `NODE_ENV`.                                                                                | `tests/unit/action-registry-schema.test.ts` (the `EXECUTABLE` set and the loop pinning all three `google_sheets.renewal_checklist.*` entries — read, reconcile, writeback — to `production_allowed:false`), `tests/unit/seed-action-registry-allowlist.test.ts`, `tests/unit/migration-readiness.test.ts`, `tests/unit/execution-risk-policy.test.ts`, `tests/unit/sheet-writeback-service.test.ts`, `tests/unit/lease-renewal-sheet-writeback-execution.test.ts`, `tests/unit/lease-renewal-writeback-safety.test.ts`, plus the preview/receipt/reconcile/rollback tests in AC-S53-12 |
...
- **Owner dependency (the one flip per row).** Named per row in Tables A and B: the `KB Proposed —
Comp basis` column plus sheet/tab confirmation; the pmikcmetro.com DWD subject mailbox for
  `KB_APPROVAL_SENDER`; two Secret Manager secrets plus the accessor binding; one vendor company and
  contact address; one `roles/discoveryengine.admin` grant. Cross-cutting S52 eligibility and the
...
- _Answered 2026-07-29 (D32):_ the `KB Proposed — Comp basis` column and exact sheet/tab confirmation
  are required now; they remain a named owner input until a fresh authenticated live read verifies them.
  Routing the write-back through its gate is a prerequisite of the first flip.
...
- Decision-complete: no product choice is open. Only the five named owner inputs and the routine reviewed
  flips remain.
...
- **AC-S53-11** — The Production deploy plan contains explicit `ENVIRONMENT_KIND=production` and
  ...
  `npm test -- tests/unit/cutover-readiness-golden.test.mjs`,
  `npm test -- tests/unit/environment-descriptor.test.ts`.
- **AC-S53-12** — Sheet write-back confirmation is immutable, one-attempt, and correctable. Resolve
  returns a server-issued preview and hash binding actor UID, run id, source trigger, spreadsheet id,
  tab, A1 cell, proposed value, approval version, and environment descriptor. Commit requires the
  byte-identical unexpired hash; actor/approval/value/target/environment drift, a missing hash, or
  `confirm:true` alone returns 409 before constructing or reading a writer. A winning idempotency key
  permits one update, persists one bodyless receipt, reads the cell back, and makes a duplicate return
  that receipt without a second update. A correction action may clear only the exact cell/value written
  by that receipt after its own exact preview/confirmation and readback; any intervening change refuses.
  _Verify:_ `npm test -- tests/unit/sheet-writeback-service.test.ts`,
  ...
  `npm test -- tests/unit/lease-renewal-writeback-safety.test.ts`.
...
`google_sheets.renewal_checklist.writeback` before its gate call, descriptor fence, immutable
preview-hash confirmation, idempotent receipt/readback/reconcile, and correction contract are in code
and proven refusing stale or boolean-only confirmation; NEVER flip
...
10. _Owner:_ hand back ONE redacted packet with the five named inputs: the `KB Proposed — Comp basis` column
    plus the confirmed spreadsheet id and tab name; the pmikcmetro.com DWD subject mailbox for
    `KB_APPROVAL_SENDER`; `MAINTENANCE_INTAKE_TOKEN_SECRET` and `MAINTENANCE_INTAKE_IP_HASH_SALT` in Secret
    Manager plus the accessor binding; one vendor company and contact address; and the
    `roles/discoveryengine.admin` grant on the runtime service account. Each input names the wrapper change
    that ships with it.
11. _Gate:_ only after each row's own contract and owner input are green, apply the flip recipe for
    that row. Write-back may flip only after AC-S53-12; comp screenshot only after AC-S53-13; each
    Vendor key only after AC-S53-8. Update the seed, both `EXECUTABLE_ALLOWLIST` copies, and pinned
