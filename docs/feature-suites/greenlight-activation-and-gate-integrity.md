...
gate exists to force (D02). The first local slices routed the Sheet write-back through its exact gate
and environment fence, built its action contract to the D32 seam, and closed the original
provisioned-but-unforwarded, wrong-flag, missing-flag, legacy-descriptor, and always-failing-control
defects without flipping a protected key. S53.5 builds the reachable Live Vendor lifecycle while its
three named keys remain observably closed; the remaining S53 work activates each eligible row only
after its exact dependency is green. When S53 is done, every capability that reports itself active is
active, every capability that is not granted refuses observably, and the owner can read the full
activation surface off one table.
...
- **The one systemic constraint every activation obeys.** `scripts/deploy-demo-cloud-run.mjs` builds the
  ...
  allowlist. Secrets are wired by one complete `readRuntimeSecrets(...)` map: RentVine binds only when
  its base URL activates that provider, while public maintenance intake binds only from the complete
  pair of explicit non-secret `*_SECRET_ID` references. A partial pair, version-only reference, or
  plaintext-only deploy configuration is refused; when no declared group is active the wrapper emits
  `--clear-secrets` so stale bindings cannot survive. Therefore **no pending credential or value can
  reach the running service without a paired code change to the deploy wrapper.** Creating a Secret
  Manager secret, setting a value in `.env.local`, or granting a scope is necessary and never
  sufficient. The executable production path prepares, preflights, and deploys from the same
  explicitly named `.env.production.local`; a missing reviewed file refuses, and stale ambient
  runtime values are not merged into its replacing maps. A value set only by a direct
  `gcloud run services update` is therefore reverted by the next reviewed deploy. Every activation
  row below names its wrapper change, or states NONE and proves it with a test. The wrapper is
  security-sensitive and every env/secret change stays paired with its forwarding/refusal tests, but
  it is not one of D12's six protected paths.

- **Gate-integrity defects this suite closes before it grants anything.**
  - **G1 — CLOSED locally (S53.1/S53.2).** The route and service now assert the exact Sheet action key
    plus the Production+Live descriptor before constructing a provider, reading a Sheet, or persisting
    an attempt. The key stays closed until the D32 provider contract and exact operating target exist.
  - **G2 — CLOSED locally (S53.4).** A Node-compatible requirement projection mirrors the committed
    per-key Registry state, with a parity sentinel against `isActionExecutable`. Both production
    preflight and deploy planning require exactly one syntactically valid managed
    `KB_APPROVAL_SENDER` plus a same-project `GMAIL_DWD_SA` whenever
    `internal.transactional_notice.send` is executable, even when the unrelated legacy digest flag is
    false. Migration readiness separately reports a governance-open action as runtime-inert when a
    dependency is blank or invalid and blocks Firestore-vs-seed production-gate drift over the
    complete key union, not only the runtime-mapped actions.
  - **G3 — CONFIG HALF CLOSED locally (S53.4).** `readServerConfig()` now exposes
    `spaceProvisioningEnabled`, false unless the value is exactly `true`/`1`, and the replacing deploy
    map forwards `SPACE_PROVISIONING_ENABLED` with a false default. The S36 Admin provision route and
    its per-Space confirmation/IAM behavior remain an S36 implementation slice; configuration work
    alone does not satisfy all of AC-S53-9 or grant `roles/discoveryengine.admin`.
  - **G4 — CLOSED locally (S53.3).** The comp control is absent while the key is closed, and the local
    AC-S53-13 preview/confirm/receipt/reconcile/trash-rollback contract is built. Its protected action
    key remains closed pending live prerequisites and review.
  - **G5 — CLOSED locally (S53.4).** Production deploy planning now forces explicit
    `ENVIRONMENT_KIND=production` and `DATA_CONTEXT=live`; production preflight rejects a missing pair
    as `legacy-node-env`, rejects every non-Production+Live pair, and accepts normalized explicit values.
...
| -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `google_sheets.renewal_checklist.writeback`  | Appends one approved value to one empty `KB Proposed — <field>` cell. The local immutable contract is built, but activation stops before provider construction because the D32 transaction provider does not exist yet. Fixed-A1/value compare-and-set is insufficient. That provider must expose one globally key-bound payload ledger with atomic stable-row mutation at the exact confirmed A1, exact status, atomic tombstone-if-absent, immutable effect evidence, and a current-cell generation invalidated by every edit (including same-value ABA). | Approve/provision the concrete D32 transaction broker + protected target/ledger ranges described in `docs/client-asks-2026-07-29.md`; create the `KB Proposed — Comp basis` column; and confirm the exact spreadsheet id and tab name. | The wrapper forwards `LEASE_RENEWAL_SHEET_WRITEBACK_ENABLED`, `RENEWAL_SHEET_ID`, `SHEETS_IMPERSONATE_SA`, `SHEETS_DWD_SUBJECT`, and now forces explicit `ENVIRONMENT_KIND=production` plus `DATA_CONTEXT=live`; the broker deployment id/secret is added only after its documented interface exists.              | `tests/unit/action-registry-schema.test.ts`, `tests/unit/seed-action-registry-allowlist.test.ts`, `tests/unit/migration-readiness.test.ts`, `tests/unit/execution-risk-policy.test.ts`, `tests/unit/google-sheets-write-client.test.ts`, `tests/unit/sheet-writeback-contract.test.ts`, `tests/unit/sheet-writeback-service.test.ts`, `tests/unit/sheet-writeback-ui-contract.test.tsx`, `tests/unit/sheet-writeback-runtime-boundary.test.mjs`, `tests/unit/lease-renewal-sheet-writeback-execution.test.ts`, `tests/unit/lease-renewal-writeback-safety.test.ts`, and `tests/firestore/lease-renewal-writeback-execution-store.test.ts`                                                                                                                                                                                                                                                                                                                                                                                                              |
| `google_drive.renewal_comp_screenshot.store` | Stores one operator-selected comp screenshot in an in-boundary Drive folder. AC-S53-13 is built locally: the first POST is preview-only; exact confirmation wins one durable attempt; recovery reuses one reserved Drive id; receipt/readback/reconcile never re-upload; and a separately confirmed rollback verifies trash of only the receipted file. The committed key remains closed and the closed control remains hidden until its protected activation review and live prerequisites.                                                                | NONE new for the folder. Owner chose to reuse the existing maintenance photo folder rather than create a second one (D31); that choice does not waive the action contract or the protected activation review.                          | `RENEWAL_COMP_DRIVE_FOLDER_ID` and optional `RENEWAL_COMP_SHARED_DRIVE_ID` now survive the wrapper's replacing environment map. An empty override reuses `MAINTENANCE_PHOTO_DRIVE_FOLDER_ID`; an empty Shared Drive id permits only a DWD-subject-owned My Drive folder, while a configured id must match exactly. | `tests/unit/action-registry-schema.test.ts`, `tests/unit/seed-action-registry-allowlist.test.ts`, `tests/unit/migration-readiness.test.ts`, `tests/unit/execution-risk-policy.test.ts`, `tests/unit/renewal-comp-screenshot.test.ts`, `tests/unit/renewal-comp-screenshot-contract.test.ts`, `tests/unit/renewal-comp-screenshot-service.test.ts`, `tests/unit/renewal-comp-screenshot-route.test.ts`, `tests/unit/renewal-comp-screenshot-drive-provider.test.ts`, `tests/unit/renewal-comp-screenshot-folder-boundary.test.ts`, `tests/unit/renewal-progress-controls.test.tsx`, `tests/unit/server-config.test.ts`, `tests/unit/live-cost-scripts.test.mjs`, and `tests/firestore/lease-renewal-comp-screenshot-execution-store.test.ts`                                                                                                                                                                                                                                                                                                            |
| `vendor.account.invite`                      | Creates one scoped Firebase Vendor principal and delivers one setup link after Admin exact confirmation. S53.5 makes the path Admin-reachable and non-Test, with deterministic identity, exact Message-ID/recipient readback, one-time fragment-to-body setup, generation-bound reissue, bodyless evidence, and no autonomous send. The key remains closed pending its named owner input and protected activation review.                                                                                                                                   | Name one vendor company and one vendor contact address in the owner packet — never in git, never in this file (D34).                                                                                                                   | NONE new. Setup-link delivery reuses the already-forwarded `KB_APPROVAL_SENDER` plus exact same-project `GMAIL_DWD_SA`; the runtime requires that named variable and does not accept the Sheets impersonation identity as a fallback.                                                                              | `tests/unit/action-registry-schema.test.ts`, `tests/unit/seed-action-registry-allowlist.test.ts`, `tests/unit/migration-readiness.test.ts`, `tests/unit/execution-risk-policy.test.ts`, `tests/unit/vendor-live-lifecycle-route.test.ts`, `tests/unit/vendor-live-lifecycle-service.test.ts`, `tests/unit/vendor-live-lifecycle-runtime.test.ts`, `tests/unit/vendor-live-lifecycle-panel.test.tsx`, `tests/unit/vendor-live-lifecycle-page.test.tsx`, `tests/unit/live-vendor-lifecycle-provider.test.ts`, `tests/unit/live-vendor-lifecycle-adapters.test.ts`, `tests/unit/live-vendor-lifecycle-import-boundary.test.ts`, `tests/unit/live-vendor-lifecycle-adapters-import-boundary.test.ts`, `tests/unit/live-vendor-setup.test.ts`, `tests/unit/live-vendor-setup-store.test.ts`, `tests/unit/live-vendor-setup-route.test.ts`, `tests/unit/live-vendor-setup-bridge.test.tsx`, `tests/unit/live-vendor-setup-import-boundary.test.ts`, `tests/unit/route-auth-boundary.test.ts`, and `tests/firestore/vendor-lifecycle-execution-store.test.ts` |
| `vendor.assignment.change`                   | Assigns or removes exactly one Vendor on exactly one maintenance ticket. S53.5 closes the generic Live `vendor-assign` bypass and wires the Admin path through exact preview, S20 confirmation, one atomic Vendor/ticket/assignment effect, bodyless receipt, and reconciliation. The key remains closed pending the same first-Vendor input and protected activation review.                                                                                                                                                                               | Same first-vendor naming as above; no separate input.                                                                                                                                                                                  | NONE new (asserted by the same wrapper-allowlist test).                                                                                                                                                                                                                                                            | `tests/unit/action-registry-schema.test.ts`, `tests/unit/seed-action-registry-allowlist.test.ts`, `tests/unit/migration-readiness.test.ts`, `tests/unit/execution-risk-policy.test.ts`, `tests/unit/vendor-live-lifecycle-route.test.ts`, `tests/unit/vendor-live-lifecycle-service.test.ts`, `tests/unit/vendor-live-lifecycle-runtime.test.ts`, `tests/unit/vendor-live-lifecycle-panel.test.tsx`, `tests/unit/vendor-live-lifecycle-page.test.tsx`, `tests/unit/live-vendor-lifecycle-provider.test.ts`, `tests/unit/live-vendor-lifecycle-import-boundary.test.ts`, `tests/unit/external-execution-s20-bridge.test.ts`, `tests/unit/maintenance-vendor-executors.test.ts`, `tests/unit/maintenance-tickets.test.ts`, and `tests/firestore/vendor-lifecycle-execution-store.test.ts`                                                                                                                                                                                                                                                                |
| `vendor.account.disable`                     | Disables one exact Vendor principal, revokes sessions, atomically clears bounded ticket assignments, and queues mailbox-token revocation. It is the reversible cutoff for the two rows above. S53.5 adds access-first crash recovery with one leased Firebase-completion owner, exact Auth readback, immutable root/child receipts, and read-only provider reconciliation. The key remains closed pending the same first-Vendor input and protected activation review.                                                                                      | Same first-vendor naming as above; no separate input.                                                                                                                                                                                  | NONE new (asserted by the same wrapper-allowlist test).                                                                                                                                                                                                                                                            | `tests/unit/action-registry-schema.test.ts`, `tests/unit/seed-action-registry-allowlist.test.ts`, `tests/unit/migration-readiness.test.ts`, `tests/unit/execution-risk-policy.test.ts`, `tests/unit/vendor-live-lifecycle-route.test.ts`, `tests/unit/vendor-live-lifecycle-service.test.ts`, `tests/unit/vendor-live-lifecycle-runtime.test.ts`, `tests/unit/vendor-live-lifecycle-panel.test.tsx`, `tests/unit/vendor-live-lifecycle-page.test.tsx`, `tests/unit/live-vendor-lifecycle-provider.test.ts`, `tests/unit/live-vendor-lifecycle-adapters.test.ts`, `tests/unit/live-vendor-lifecycle-import-boundary.test.ts`, `tests/unit/live-vendor-lifecycle-adapters-import-boundary.test.ts`, `tests/unit/external-execution-s20-bridge.test.ts`, `tests/unit/maintenance-vendor-executors.test.ts`, `tests/unit/maintenance-tickets.test.ts`, and `tests/firestore/vendor-lifecycle-execution-store.test.ts`                                                                                                                                      |
...
| -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `KB_APPROVAL_SENDER`, `GMAIL_DWD_SA`                                 | The internal transactional sender and its keyless DWD service identity. `internal.transactional_notice.send` is governance-open but correctly reported runtime-inert while either exact value is absent/invalid (D29). | Supply the exact existing managed `@pmikcmetro.com` DWD subject; never infer it. `GMAIL_DWD_SA` already exists and must be exactly one service account in `GCP_PROJECT_ID`; this slice requests no new identity or scope.                                                                                                                                          | COMPLETE locally: preflight, preparation, deploy planning, readiness, and the concrete sender require one dot-atom managed mailbox; the action-runtime projection also requires the same-project DWD service identity, independent of the legacy digest flag.                                                              | `tests/unit/action-runtime-requirements.test.mjs`, `tests/unit/cutover-readiness-golden.test.mjs`, `tests/unit/live-cost-scripts.test.mjs`, `tests/unit/internal-transactional-sender.test.ts`, `tests/unit/migration-readiness.test.ts`, `tests/unit/report-issue-route.test.ts`    |
| `MAINTENANCE_INTAKE_TOKEN_SECRET`, `MAINTENANCE_INTAKE_IP_HASH_SALT` | The HMAC signing secret and the IP-hash salt for the public tokenized resident intake. Either missing/weak half or a reused value keeps web/CLI mint and public submit unavailable (D30, `F-MAINT-INTAKE-PUBLIC`).     | Create two distinct generated values of at least 32 UTF-8 bytes under two different Secret Manager ids and grant the Cloud Run runtime SA `roles/secretmanager.secretAccessor`. Cost eligibility is determined by S52; this table makes no zero-cost claim.                                                                                                        | COMPLETE locally: the complete pair of different explicit `*_SECRET_ID` references joins the wrapper's one exact `--set-secrets` map; partial/version-only/plaintext-only/same-id deploy configuration is refused, and runtime health separately validates both resolved values without requiring ids inside the revision. | `tests/unit/runtime-secret-bindings.test.mjs`, `tests/unit/maintenance-intake-public-route.test.ts`, `tests/unit/maintenance-intake-token-route.test.ts`, `tests/unit/maintenance-intake-token.test.ts`, `tests/unit/live-cost-scripts.test.mjs`, `tests/unit/server-config.test.ts` |
| `SPACE_PROVISIONING_ENABLED` + `roles/discoveryengine.admin`         | Lets an Admin self-provision a Space behind a per-Space cost confirm. The configuration seam is built; the S36 route and IAM grant are still required (D36).                                                           | Grant `roles/discoveryengine.admin` to the runtime service account named in the owner packet, then set the flag true only after S36's provision route is built and S52 is eligible.                                                                                                                                                                                | CONFIG COMPLETE locally: `readServerConfig()` resolves false by default and the wrapper forwards the reviewed value. The S36 provision-route tests remain outstanding; no route or IAM grant is implied.                                                                                                                   | `tests/unit/server-config.test.ts`, `tests/unit/live-cost-scripts.test.mjs`, plus the S36 provision-route test when that slice lands                                                                                                                                                 |
| `ENVIRONMENT_KIND`, `DATA_CONTEXT`                                   | Makes the S40 descriptor explicit in Production instead of derived from `NODE_ENV`, so the new write-back fence and every other descriptor guard rest on configuration (G5).                                           | NONE. These are deployment configuration, not a credential or a grant.                                                                                                                                                                                                                                                                                             | COMPLETE locally: the production wrapper forces the exact pair, preparation writes it, and production preflight refuses `legacy-node-env`, a partial pair, or anything other than `production` + `live`.                                                                                                                   | `tests/unit/environment-descriptor.test.ts`, `tests/unit/cutover-readiness-golden.test.mjs`, `tests/unit/live-cost-scripts.test.mjs`, `tests/unit/prepare-production-env.test.mjs`                                                                                                   |
...
| Not granted                                                                                                                                                                            | Reason                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `gmail.renewal_notice.send`, `gmail.maintenance_owner_notice.send`                                                                                                                     | D33. There is no reachable Live sending path behind either gate, so flipping them would be meaningless or dangerous. Draft-into-Gmail with a human sending from Gmail IS the end state, not a waypoint. Their committed Registry rows are `Disabled`; retained typed executors, preview schemas, or isolated Test receipts are historical contract evidence only, never an activation seam. Only a new explicit owner decision that supersedes D33 could change these permanent non-targets. |
...
- **Completed locally (app-plane).** G1's gate/descriptor fence and AC-S53-12 seam; G2's action-keyed
  sender requirement; G3's fail-closed config/forwarding half; G4 plus AC-S53-13; G5's explicit
  descriptor; the current Table A/B forwarding sentinel; paired intake-secret planning; and the
  readiness projection that preserves raw `production_allowed` truth while separately naming
  runtime-active and runtime-inert mapped actions. S53.5 also completes AC-S53-8's Admin-reachable
  Vendor invite, assignment, disable, setup, receipt, reconciliation, and rollback seam without
  opening any named Vendor key. The S36 provision route is not part of this completed list.

- **Built to the seam (live provider).** The Live provider is reachable only from explicit
  Production+Live Admin controls and no longer depends on the synthetic release harness or Test workflow.
  Every mutation crosses the named Registry gate before runtime construction, reloads immutable current
  source generations, and uses S20 preview/confirm authority. Invite uses deterministic Firebase identity,
  exact Gmail Message-ID/recipient readback, and a one-time body-only setup challenge. A Vendor-level
  transaction fence serializes setup effects against reissue and disable; a lost post-effect store write
  stays fail-closed for Admin review rather than minting a second reset credential. Disable first commits
  the reversible Firestore cutoff, then gives one bounded lease owner the Firebase completion sequence.
  The real Firestore emulator proves the exact 164-assignment/500-write boundary and rejects 165 with no
  partial effect. The generic `vendor-assign` PATCH path cannot mutate Live data. S46 may now remove its
  Test selector only when its own two-stage retirement evidence is green.
...
- **Owner dependency (the one flip per row).** The six named inputs in Tables A and B are: (1) the D32
  ...
  protected `production_allowed` review still apply. The Sheet and Drive local action contracts in
  AC-S53-12/13 are complete; both keys remain closed on their exact external/live prerequisites and
...
- _Answered 2026-07-29 (D31):_ reuse `MAINTENANCE_PHOTO_DRIVE_FOLDER_ID` for comp screenshots and do
  ...
  confirmation, idempotency, receipt, reconciliation, and rollback invariants. AC-S53-13 is built
  locally; the key stays closed pending its exact live prerequisites and protected activation review.
...
- _Assumption:_ the two `EXECUTABLE_ALLOWLIST` copies stay deliberately non-identical — the seed script's
  ...
  flipping a record.
- _Resolved 2026-07-30:_ preserve raw governance state and surface an executable action with a
  missing/invalid runtime dependency as `runtime-inert`, naming every required variable and adding a
  readiness blocker. Committed seed authority drives this projection; drift in returned Firestore
  metadata is a separate named blocker.
...
`app/api/admin/spaces/request/route.ts`, `app/admin/vendors/page.tsx`,
`app/api/admin/vendors/live/actions/route.ts`, `app/vendor/setup/page.tsx`,
`app/api/vendor/setup/route.ts`, `components/admin/LiveVendorLifecyclePanel.tsx`,
`components/vendor/VendorSetupBridge.tsx`, `lib/vendor/live-lifecycle-*`,
`lib/vendor/live-setup*`, `lib/firestore/vendor-lifecycle-executions.ts`, and `.env.example`.
Interacts with `F-GREENLIGHT-NAMED-KEYS`
(this suite owns its activation table), `F-EXTERNAL-ACTION-GATE`, `F-SEND-AUTHORIZED`,
`D-AUTOMATION-LINE`, `F-INTERNAL-NOTIFY` (whose governance-open action is now separately reported
runtime-inert until the mailbox exists), `F-MAINT-INTAKE-PUBLIC` (fail-closed until both runtime
secrets exist), `F-DRIVE-DWD` (the Drive scope is already authorized, so the comp-screenshot flip
needs no new grant), `F-OWNER-DEPS-2026-07-23` (the `roles/discoveryengine.admin` grant remains the
S36 activation input), and
...
selector.** Delete-on-supersede: the former Sheet `readiness:"Planned"` posture is now
`"Needs Connection"`; the G3 missing-flag and G5 legacy-descriptor claims are replaced by
`F-S53-RUNTIME-CONFIG-INTEGRITY`; and D33 retires the S38b client-send follow-on in favor of
draft-into-Gmail plus a human send. Each has a `docs/facts.md` Supersede Log marker. The Sheet
`production_allowed:false` posture remains truthful until its D32 dependency and protected
activation review complete. Governance: no `firestore.rules` shape change; a `production_allowed`
seed change is the D12 protected path prepared for owner review. Its paired allowlist,
deploy-wrapper, and test edits travel in the same activation review package but are not independently
D12-protected.
...
- **AC-S53-4** — Provisioned-but-unforwarded cannot be reported active. A test enumerates every name in
  ...
  `scripts/deploy-demo-cloud-run.mjs`; preflight and deploy consume the same named reviewed production
  env file, and a missing file refuses rather than falling back to `.env.local` or ambient runtime
  settings. When a required name is absent from the wrapper, `buildDemoDeployCommand` returns
  `ok:false` with that variable named in `errors`, and the readiness projection renders the capability
  as inert with the missing variable named rather than as active. Removing a forwarded name from the
  wrapper turns this check RED. _Verify:_
  `npm test -- tests/unit/live-cost-scripts.test.mjs`,
  `npm test -- tests/unit/migration-readiness.test.ts`.
  **Status 2026-07-30: built locally for the current Table A/B surface.**
- **AC-S53-5** — With `internal.transactional_notice.send` executable and `KB_APPROVAL_SENDER` blank,
  ...
  valid dot-atom `@pmikcmetro.com` subject mailbox set, filing one support report produces exactly one
  delivered internal notice and one receipt. Multiple or malformed syntax (including punctuation that
  Gmail DWD will reject) refuses before client construction. Runtime-active truth additionally
  requires `GMAIL_DWD_SA` to be exactly one valid service account in `GCP_PROJECT_ID`. _Verify:_
  `npm test -- tests/unit/cutover-readiness-golden.test.mjs`,
  `npm test -- tests/unit/live-cost-scripts.test.mjs`,
  `npm test -- tests/unit/internal-transactional-sender.test.ts`; keep
  `tests/unit/report-issue-route.test.ts` green.
  **Status 2026-07-30: built locally; the exact mailbox value remains an owner input.**
- **AC-S53-6** — `POST /api/maintenance/intake/public` returns 503 with the generic body
  ...
  The two deploy references must name different Secret Manager ids. Both resolved runtime values must
  be distinct and at least 32 UTF-8 bytes; weak, reused, or partial values produce the same generic web
  503 before minting, verification, or persistence, and the documented CLI mint path refuses the same
  invalid pair.
  ...
  **Status 2026-07-30: built locally; the two secrets and accessor grant remain owner inputs.**
- **AC-S53-7** — While `google_drive.renewal_comp_screenshot.store` is closed, the renewal desk
  renders NO comps-screenshot file input. With no
...
- **AC-S53-8** — An Admin-reachable, non-Test path previews and — only after exact confirmation against the
  ...
  it. Setup challenges are one-time, body-only, generation-bound capabilities; reissue and disable
  serialize against an in-flight setup effect. Disable recovery gives only one lease owner the Firebase
  completion sequence, while reconcile remains provider-read-only. The real Firestore transaction accepts
  the calculated 164-assignment/500-write boundary and rejects 165 without a partial effect. Closed keys
  render truthful disabled controls and refuse before runtime construction. _Verify:_
  `npm test -- tests/unit/vendor-live-lifecycle-route.test.ts
tests/unit/vendor-live-lifecycle-service.test.ts
tests/unit/vendor-live-lifecycle-runtime.test.ts
tests/unit/vendor-live-lifecycle-panel.test.tsx
tests/unit/vendor-live-lifecycle-page.test.tsx
tests/unit/live-vendor-lifecycle-provider.test.ts
tests/unit/live-vendor-lifecycle-adapters.test.ts
tests/unit/live-vendor-lifecycle-import-boundary.test.ts
tests/unit/live-vendor-lifecycle-adapters-import-boundary.test.ts
tests/unit/live-vendor-setup.test.ts tests/unit/live-vendor-setup-store.test.ts
tests/unit/live-vendor-setup-route.test.ts tests/unit/live-vendor-setup-bridge.test.tsx
tests/unit/live-vendor-setup-import-boundary.test.ts
tests/unit/external-execution-s20-bridge.test.ts
tests/unit/maintenance-vendor-executors.test.ts tests/unit/maintenance-tickets.test.ts
tests/unit/route-auth-boundary.test.ts`; run `npm run test:firestore`, whose pinned glob includes
  `tests/firestore/vendor-lifecycle-execution-store.test.ts`.
  **Status 2026-07-30: built locally; all three named Vendor keys remain closed pending their exact
  owner input, runtime/auth proof, and protected activation review.**
- **AC-S53-9** — `readServerConfig()` exposes `SPACE_PROVISIONING_ENABLED` resolving false by default, the
  ...
  `npm test -- tests/unit/live-cost-scripts.test.mjs`.
  **Status 2026-07-30: config and forwarding complete; the S36 provision route portion remains open.**
...
- **AC-S53-11** — The Production deploy plan contains explicit `ENVIRONMENT_KIND=production` and
  ...
  `npm test -- tests/unit/cutover-readiness-golden.test.mjs`,
  `npm test -- tests/unit/environment-descriptor.test.ts`.
  **Status 2026-07-30: built locally.**
...
3. _Build — COMPLETE locally:_ close G1 — add `assertActionExecutable` and
   `assertLiveProviderActionAllowed` to the
...
4. _Build — COMPLETE locally:_ replace boolean-only Sheet confirmation with AC-S53-12's immutable server-preview hash,
   one-attempt idempotency, receipt/readback/reconcile, and guarded correction. The gate remains closed.
5. _Build — COMPLETE locally:_ close G5 and G3's config half — force
   `ENVIRONMENT_KIND`/`DATA_CONTEXT`, refuse `legacy-node-env` in cutover preflight, and add
   `SPACE_PROVISIONING_ENABLED` (default false) to `lib/config/server.ts` and `readRuntimeEnv`
   (AC-S53-11 and the config half of AC-S53-9).
6. _Build — COMPLETE locally:_ close G2 — key the `KB_APPROVAL_SENDER` requirement to the committed
   executable-action projection and generalize the mapping for future sender-dependent keys (AC-S53-5).
7. _Build — COMPLETE locally:_ land the current Table A/B deploy-forwarding sentinel and readiness
   inertness projection (AC-S53-4), plus paired intake-secret activation/refusal (AC-S53-6).
8. _Build — COMPLETE locally:_ close G4, add the folder fallback, and land AC-S53-13's
   preview/confirm/idempotency/receipt/reconcile/trash rollback contract. The key remains closed.
9. _Build — COMPLETE locally:_ implement the live `VendorLifecycleProvider` behind
   `VendorLifecycleExecutor` with preview,
   exact confirmation, one-attempt idempotency, bodyless receipt, reconcile, and disable-as-rollback, plus
   the negative-import proof that it does not depend on anything S46 removes (AC-S53-8). The
   completed contract persists the full immutable prepared attempt before S20 claim; binds provider
   start to the exact S20 claimant, canonical command/preview/payload, deterministic references, and
   predecessor lineage; and atomically fences a no-provider reconciliation against provider start.
   Invite separately claims irreversible Firebase-principal and Gmail-delivery effects, reconciles
   only by exact RFC Message-ID plus recipient, permits setup completion to race historical receipt
   closure, and records positive delivery after a disable cutoff as terminal
   `delivery_invalidated` evidence rather than pending setup or no-effect. Disable is access-first,
   uses a deterministic disabled Firebase tombstone to fence an in-flight invite, grants one
   hash-proven raw-token worker the disable/revoke sequence, verifies the representable Firebase
   revocation boundary, and permits read-only crash reconciliation to close only the exact current
   root/recovery lineage. Persisted terminal success passes one strict parser that recomputes
   deterministic receipt/provider references, execution links, immutable bindings, canonical
   timestamps, exact field sets, and per-state bodyless result hashes before runtime/Firestore
   hydration or S20 result generation. The optimistic transaction harness runs callbacks on
   overlapping snapshots and retries commit conflicts; serial callback execution is not accepted as
   race proof.
...
11. _Gate:_ only after each row's own contract and owner input are green, apply the flip recipe for
    ...
    column/id/tab. The comp contract is complete but its key waits for live prerequisites and protected
    activation review; each Vendor key waits for AC-S53-8. Update the
...
12. _Verify:_ run AC-S53-1 through AC-S53-14 and explicitly falsify: an ungated write-back call, a
    ...
    reported executable, a Vendor path that imports a surface S46 deletes, prepared-attempt
    persistence racing S20 claim, provider start racing a no-effect fence, competing identity/Gmail
    effect claims, duplicate disable workers, setup completion racing delivery receipt closure,
    disable racing a positive delivery readback, and fractional-second Firebase revocation evidence.
13. _Context update:_ promote the shipped work to `docs/facts.md` `F-*` rows citing the `AC-S53-*` ids
    ...
    impacts, record the resolved runtime-inert projection in the fact ledger, and advance
