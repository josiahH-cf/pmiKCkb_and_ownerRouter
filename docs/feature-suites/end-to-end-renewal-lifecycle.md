...
| Downstream work bound to the current proposal version   | Already satisfied | `ownerDecisionRevision`, S97 proposal generation claims (`lib/firestore/s97-renewal-writeback-claim.ts`), S98 lease-scoped claim, S66 packet snapshot hash; since 2026-09-06 `ownerOutcomeBlocksDownstream` refuses the tenant draft, S97 execution, and the S98 append while the recorded owner response is a revision request, a decline, or unanswered, and every later plan carries the recorded response forward |
