...
Status: Current working-V1 identity strategy (reconciled 2026-08-03). This began as the
...
2026-08-03 environment update: Production is Live-only. There is no current product Test workspace,
hosted Demo project, or fixture seeder. Local rehearsal resolves exactly
`environmentKind:"demo"`, `dataContext:"live_readonly"`, and `source:"explicit"`; it can perform
bounded Live reads but refuses every durable write and provider effect. Production accepts only real,
assigned Live Vendor identities and mailboxes. Former synthetic/Test Vendor workflows are dated
contract evidence and deterministic automated-test fixtures only.
...
scripts, demo-operator's old project-number URL). The current Live-only app runs as `pmi-kc-app` on
`pmi-kc-kb-prod`; the separately hosted Demo contemplated later was never provisioned and remains
deferred. The legacy GCP project was **deleted 2026-06-20** (`DELETE_REQUESTED`, recoverable
...
**Owner mandate:** every internal human, connector, cloud, build, runtime, and delegated-Workspace
surface uses a `pmikcmetro.com` or `pmi-kc-kb-prod` identity; the personal
`josiah.abernathy@gmail.com` account is never in any auth path. A real assigned external Live Vendor
is the narrow separate principal described in §2.1 and never inherits an internal identity or its
authority.

- **One organizational domain for internal access.** All internal human, connector, and machine
  identities resolve to `pmikcmetro.com` (org `584930494337`, prod project `pmi-kc-kb-prod`
  `558870356522`). No `cherrybridge.ai` (legacy demo `pmikckb-test` / `800237451321`) and no personal
  `gmail.com`/`googlemail.com` account appears in an internal Production path. A real external Live
  Vendor remains isolated to the separately scoped assigned-ticket identity and same-address mailbox
  boundary.
...
- **Fail loud, never silent.** "Blocked on access" must surface as an explicit, attributable
  error — never a fallback to local rehearsal or a personal account.
...
A missing `scopes` claim is the backward-compatible ALL-spaces wildcard, so existing provisioned
staff accounts need no claim migration. A present claim must be a non-empty array of known values;
...
identity. The narrow V1 exception is an Admin-invited, real assigned Live Vendor with password setup,
verified-email TOTP before ticket detail, and assigned-ticket-only authorization. A Live Vendor
connects the same verified routable Gmail/Google Workspace address through server-side per-vendor
OAuth. The Vendor never uses DWD or receives staff roles, Spaces, cloud/admin/connector authority,
shared/alias mailbox inference, cross-mailbox authority, or general inbox access. OAuth client,
consent, redirect, and token-vault resources activate independently for that named Vendor.

Identity class takes precedence over domain. Internal roster, last-Admin accounting, role/scope
mutation, ID-token validation, and session-cookie validation fail closed when **any** `vendor`,
`vendor_id`, or `data_mode` custom-claim key is present—even `vendor:false`, an empty id, or a malformed
mode—so partial claim drift cannot turn an external identity into staff. Canonical Live Vendor auth is
the separate positive path and still requires the exact valid `vendor:true` + canonical `vendor_id` +
matching Live `data_mode` tuple, verified email, TOTP, and the active Vendor-record join. Outside
reporters continue using the separate HMAC-token public maintenance intake; it is not the authenticated
Vendor portal.

The former canonical `.invalid` Test Vendor, Test assignments, app-only mailbox, simulated replies,
authentication-reset ceremony, and their bodyless receipts are historical contract evidence from the
retired Production Test lane. Deterministic versions remain under automated-test helpers only; no
current product route can provision, authenticate, assign, reset, or operate them. Local rehearsal is
also incapable of Vendor operation: its exact explicit Demo + Live-read-only descriptor is fenced
before identity mutation, mailbox construction, persistence, or provider effects. No hosted Demo
project or fixture seeder replaces the retired lane.
...
- Rollback: disable the workflow-facing Gmail registry keys,
  ...
  appropriate, disable the push subscription/topic, and redeploy the prior revision. The narrowly
  authorized S31 Scheduler may renew the read-only Gmail watch; no Scheduler may draft, send, or
  trigger another client-facing effect.
...
**(d) Firebase end-user:** the Production release path's preflight must pass
(`ALLOWED_HD=pmikcmetro.com`, demo flags false, `KB_APPROVAL_SENDER`/`KB_APPROVAL_RECIPIENTS` all
`@pmikcmetro.com`); deploy
...
bounded Live reads but refuses Firebase/Vendor lifecycle mutation, app persistence, and provider
effects. It does not seed an invented user or workflow and is not a hosted environment.
...
stop:** D01 retired the old flat figure. S52's applied, read-back values are a `$25` warning-only
alert and a `$100` lockstep budget/guardrail hard stop. Cost-bearing work still requires its
path-specific preflights and current live readback; unexpected spend halts broader work.

## Immediate next actions for Josiah

1. **Activate only a named real Live Vendor:** bind the verified assigned Vendor identity and
   same-address mailbox through the exact invite/TOTP/assignment/OAuth contracts. Never retain a setup
   link, password, TOTP seed, recovery code, token, or mailbox body in repository evidence.
...
3. **Activate Live providers per exact action:** add the documented identity, mapping, credential,
   ...
   activation inputs leave only that Live action unavailable; they do not make the Live application
   incomplete or authorize a synthetic product fallback.
