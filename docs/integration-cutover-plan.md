...
| 3. Integration capability verification | Prove each intended integration can be used safely.      | Run non-destructive API checks and smoke tests; verify per-vendor roles, event mode, and maintenance-first order per `docs/integration-architecture.md`. | Approve scopes, senders, test data, and sandbox accounts.                   | Each integration is verified, rejected, or moved to research backlog.   |
...
| Lease Renewal Agent | No standalone agent runtime should be built yet; the Rentvine lease-renewal writeback is undocumented and stays gated. | Requirements, permitted systems, trigger model, data source list, user approval model, and acceptance tests.                   |
...
- Catalog every external action type in the Action Registry before requesting executable
  approval; entries stay `production_allowed: false` until an approved spec changes them.
